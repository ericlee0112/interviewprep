...
Write a function that receives two arguments:
    an Accept-Language header value as a string 
...
def parseAcceptLanguage(acceptLanguageHeader, supportedLanguages):
    supportedLanguagesSet = set(supportedLanguages)
    output = []
    requestedLanguagesList =  acceptLanguageHeader.split(", ")

    for language in requestedLanguagesList:
        if language in supportedLanguagesSet:
...
print(parseAcceptLanguage(acceptLanguageHeader, supportedLanguages))